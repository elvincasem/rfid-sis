...
function savepar(){
	...
	$.ajax({
		...
		success: function(response) {
			...
			window.location.href = "par/details/"+lastid;
...
function deletepar(id){
	...
	var r = confirm("Are your sure you want to delete this PAR?");
    if (r == true) {
...
function updatepardetails(){
	...
	$.ajax({
		url: '../updatepardetails',
		type: 'post',
		data: {parid:parid,employeeid:employeeid,parno:parno,pardate:pardate},
		success: function(response) {
			console.log(response);
...
function addpropertytolist(){
	...
	$.ajax({
		url: '../addpropertytolist',
		type: 'post',
		data: {parid: parid,equipno:equipno,assetid:assetid},
		success: function(response) {
			//console.log(response);
			//location.reload();
			$('#general-table').load(document.URL +  ' #general-table');
